...
lemma lvm_lemma_Add_Multiple1(lvm_b0:lvm_codes, lvm_s0:lvm_state, lvm_sN:lvm_state, 
  ...
  requires src2.D?;
  requires src2.d.Int?;
  requires lvm_require(lvm_b0, lvm_code_Add_Multiple1(dst, size,src1,src2), lvm_s0, lvm_sN)
  requires lvm_is_dst_opr(dst, lvm_s0)
...
//   
  ...
  ensures OperandContents(lvm_sM, dst).val == evalADD(OperandContents(lvm_s0,dst).itype.size,OperandContents(lvm_s0,src2),D(Int(5,src2.d.itype)).d).val
  // ensures ToTwosComp(OperandContents(lvm_sM, dst)).val == 
