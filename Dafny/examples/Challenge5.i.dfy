...
predicate challengeStateAssumptionsPred(s:state)
   ...
   predicate stateFramingMultiValue(s:state,s':state)
    ...
    // predicate MiniSpec(s:LLVM_def.state,cipherText:Data, bytes_written:Data)
    // {
    //     && ValidState(s)
    //     && forall r:LLVM_def.state :: (evalBlock(encrypt(),s,r) ==> stateFramingMultiValue(s,r,{cipherText,bytes_written})) 
    ...
    predicate RemovedBehaviors(b:behavior)
    {
        forall i,j :: 
            (&& i >= 0 
             ...
             && j > i
             && j < |b|
             && evalBlock(encrypt(),b[i],b[j])) 
             ==> stateFramingMultiValue(b[i],b[j])
    }

     function encrypt():codes
        // reads s.m.mem; plainText:Data, s:state
    {
    ...
    //main function block
        lvm_Block(lvm_Codes(Ins(ALLOCA(bytes_written,4)),
                  lvm_Codes(Ins(CALL(call,malloc(Int(0,IntType(8,false))))),
                  lvm_Codes(Ins(LLVM_MEMCPY(call,mblock,1,false)),
                  lvm_Codes(Ins(CALL(call1,malloc(call1.d))),
                  lvm_Codes(Ins(CALL(call2,encrypt())),
                  lvm_Codes(Ins(STORE(call2,bytes_written)),
                  lvm_Codes(Ins(LOAD(var_1,4,D(Ptr(0,0,0,1)))),
    ...
    lemma challengeProb5PatchBehavior(codes:lvm_codes, s:lvm_state) 
        returns (postBehavior:behavior)

        requires codes.lvm_Codes?;
        requires codes.tl.CNil?
        requires ValidState(s);
        requires challengeStateAssumptionsPred(s);

        // requires codes == challenge_prob_5_code_write_encrypted();
        requires exists s' :: lvm_require(codes,challenge_prob_5_code_write_encrypted(),s,s');

    {
        assert false;
        postBehavior := [s];
        // assert codes.hd == challenge_prob_5_code_write_encrypted();
        
        var codeBlockaa := challenge_prob_5_code_write_encrypted();
        var s' :|  lvm_require(codes, codeBlockaa, s, s');
        
        assert s.ok;
        // assert !codes.hd.Block?;
        ghost var lvm_ltmp1, lvm_cM:lvm_code, lvm_ltmp2 := lvm_lemma_block(codes, s, s');
        assert false;

        var r := lvm_ltmp1;
        var tail := lvm_ltmp2;
        assert tail == codes.tl;
        assert tail.CNil?;
        var codeBlock:lvm_codes := lvm_get_block(lvm_cM);
        
        var bytes_written :| ValidOperand(s,bytes_written) && ValidState(s) && bytes_written.D? && D(Ptr(0,0,0,1)) == bytes_written;
        assert codeBlock.hd == Ins(ALLOCA(bytes_written,4));

                

        ghost var updatedB, unwrapedCode, nextState := lvm_lemma_Alloca(postBehavior, codeBlock, r, bytes_written, 4);
        assert AllocaStep(s.m,4) == nextState.m;
        assert StateNext(s,nextState);
        postBehavior := updatedB;
        assert ValidBehavior(postBehavior) && |updatedB| == 2;

        var call := D(Ptr(0,0,0,1));
        assert unwrapedCode.hd == Ins(CALL(call,malloc(Int(0,IntType(8,false)))));

        updatedB, unwrapedCode, nextState := malloc_(postBehavior, unwrapedCode, call, Int(0,IntType(8,false)), s');
        
        var mblock := D(Ptr(0,0,0,1));
        assert unwrapedCode.hd == Ins(LLVM_MEMCPY(call,mblock,1,false));
        assert ValidBehavior(postBehavior) && |updatedB| == 3;

        updatedB, unwrapedCode, nextState := lvm_lemma_MemCpy(postBehavior, unwrapedCode, r, call, mblock,1);
        ...
        // assert forall b :: BehaviorEvalsCode(Ins(RET(D(Void))), b) ==> ValidBehavior(b);    
    }
