...
module challenge6CodeLemmasSideEffect{
...
lemma unwrapVulnBehaviors(s:state, input:operand, c:codeRe) returns (preB:behavior)
        ...
        requires c == challenge_prob_5_code_write_encrypted_simple_side_effect();

        // -- input is valid -- ie. valid 16 bit integer 
        requires validInput(s,input);
        
        // ensures ValidBehaviorNonTrivial(preB);
        ensures BehaviorEvalsCode(challenge_prob_5_code_write_encrypted_simple_side_effect(),preB);
        ensures |preB| == 10;
        ensures preB[0] == s;
...
// 
        ...
        assert preB[4] == evalInsRe(CALL(call2,encrypt_side_effects(INTEGRITY_SIZE)),preB[3]);
        assert ValidInstruction(preB[3],CALL(call2,encrypt_side_effects(INTEGRITY_SIZE)));
        var sha := unwrapEncryptSideEffectsWitness(preB[3],call2,INTEGRITY_SIZE,preB[4]);
        assert ValidState(preB[4]);
        assert ValidBehavior(sha);
        assert ValidOperand(last(sha),call2);
        assert NextStep(preB[3],preB[4],Step.evalInsStep(CALL(call2,encrypt_side_effects(INTEGRITY_SIZE))));
        // var stemp := stateUpdateVar(last(sha),call2,OperandContents(last(sha),call2));
        assert validEvalIns(CALL(call2,encrypt_side_effects(INTEGRITY_SIZE)),preB[3],preB[4]);
        assert call2.LV?;
        assert  StateNext(preB[3],preB[4]);
        assert OperandContents(preB[3],INTEGRITY_SIZE).val == 4;
        assert ValidOperand(preB[4],INTEGRITY_SIZE);
        assert OperandContents(preB[4],INTEGRITY_SIZE).val == 3;

        // assert preB[4] == last(sha).(o := Nil);
                // assert OperandContents(preB[4],INTEGRITY_SIZE).val == 3;

        // assert preB[3] == preB[4];
        assert ValidState(preB[4]);
         assert preB[5] == evalInsRe(STORE(call2,bytes_written),preB[4]);
         assert ValidOperand(preB[4],call2);// && 
         assert ValidOperand(preB[4],bytes_written);
        //  assert ValidInstruction(preB[4],STORE(call2,bytes_written));
...
//         reveal_ValidBehavior();
//         reveal_BehaviorEvalsCode();
//         var c := challenge_prob_5_code_write_encrypted_simple_side_effect();
